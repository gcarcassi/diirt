...
/**
 * Formula function that returns the value of a formula matching the name
 * of the argument.
 *
 * @author carcassi
 */
public class ChannelFormulaFunction extends DynamicFormulaFunction {
    ...
    public Object calculate(final List<Object> args) {
        ...
        if (!Objects.equals(newName, previousName)) {
            ...
            // Connect new
            if (newName != null) {
                currentExpression = ExpressionLanguage.formula(newName, true,
                        (ChannelTranslator) getDirector().getOptions().get("channelTranslator"),
                        (UserInfo) getDirector().getOptions().get("userInfo"));
